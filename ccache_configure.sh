...
if [ -e ${ccache_bin} ]; then
    ...
    echo "Cannot find ccache at ${ccache_bin}, please build and install."
    echo "$ git clone https://github.com/ccache/ccache.git"
    echo "$ cd ccache"
    echo "$ git checkout v4.2 -b 4.2"
    echo "$ build"
    echo "$ cmake -DCMAKE_INSTALL_PREFIX=$HOME -DCMAKE_BUILD_TYPE=Release -DZSTD_FROM_INTERNET=ON .."
    echo "$ make -j"
    echo "$ make install"
    exit 1
fi
